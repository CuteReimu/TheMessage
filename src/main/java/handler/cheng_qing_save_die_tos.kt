...
import com.fengsheng.handler.cheng_qing_save_die_tos
...
class cheng_qing_save_die_tos : AbstractProtoHandler<Fengsheng.cheng_qing_save_die_tos>() {
    override fun handle0(r: HumanPlayer, pb: Fengsheng.cheng_qing_save_die_tos) {
        if (!r.checkSeq(pb.seq)) {
            cheng_qing_save_die_tos.Companion.log.error("操作太晚了, required Seq: " + r.seq + ", actual Seq: " + pb.seq)
            return
        }
        if (r.game.fsm !is WaitForChengQing || r !== fsm.askWhom) {
            cheng_qing_save_die_tos.Companion.log.error("现在不是使用澄清的时机")
            return
        }
        if (!pb.use) {
            r.incrSeq()
        ...
        if (card == null) {
            cheng_qing_save_die_tos.Companion.log.error("没有这张牌")
            return
        }
        if (card.type != Common.card_type.Cheng_Qing) {
            cheng_qing_save_die_tos.Companion.log.error("这张牌不是澄清，而是$card")
            return
        }
