...
import com.fengsheng.handler.skill_guang_fa_bao_b_tos
...
class skill_guang_fa_bao_b_tos : AbstractProtoHandler<Role.skill_guang_fa_bao_b_tos>() {
    override fun handle0(r: HumanPlayer, pb: Role.skill_guang_fa_bao_b_tos) {
        val skill = r.findSkill<Skill>(SkillId.GUANG_FA_BAO)
        if (skill == null) {
            skill_guang_fa_bao_b_tos.Companion.log.error("你没有这个技能")
            return
        }
        if (HashSet(pb.cardIdsList).size != pb.cardIdsCount) {
            skill_guang_fa_bao_b_tos.Companion.log.error("卡牌重复" + Arrays.toString(pb.cardIdsList.toTypedArray()))
            return
        }
