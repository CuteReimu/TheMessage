...
import com.fengsheng.handler.choose_whether_receive_tos
...
class choose_whether_receive_tos : AbstractProtoHandler<Fengsheng.choose_whether_receive_tos>() {
    override fun handle0(r: HumanPlayer, pb: Fengsheng.choose_whether_receive_tos) {
        if (!r.checkSeq(pb.seq)) {
            choose_whether_receive_tos.Companion.log.error("操作太晚了, required Seq: " + r.seq + ", actual Seq: " + pb.seq)
            return
        }
        if (r.game.fsm !is SendPhaseIdle || r !== fsm.inFrontOfWhom) {
            choose_whether_receive_tos.Companion.log.error("不是选择是否接收情报的时机")
            return
        }
        if (pb.receive) {
            r.incrSeq()
            ...
            if (r === fsm.whoseTurn) {
                choose_whether_receive_tos.Companion.log.error("传出者必须接收")
                return
            }
            var locked = false
            ...
            if (locked) {
                choose_whether_receive_tos.Companion.log.error("被锁定，必须接收")
                return
            }
            r.incrSeq()
