...
import com.fengsheng.handler.use_feng_yun_bian_huan_tos
...
class use_feng_yun_bian_huan_tos : AbstractProtoHandler<Fengsheng.use_feng_yun_bian_huan_tos>() {
    override fun handle0(r: HumanPlayer, pb: Fengsheng.use_feng_yun_bian_huan_tos) {
        if (!r.checkSeq(pb.seq)) {
            use_feng_yun_bian_huan_tos.Companion.log.error("操作太晚了, required Seq: " + r.seq + ", actual Seq: " + pb.seq)
            return
        }
        val card = r.findCard(pb.cardId)
        if (card == null) {
            use_feng_yun_bian_huan_tos.Companion.log.error("没有这张牌")
            return
        }
        if (card.type != Common.card_type.Feng_Yun_Bian_Huan) {
            use_feng_yun_bian_huan_tos.Companion.log.error("这张牌不是风云变幻，而是$card")
            return
        }
