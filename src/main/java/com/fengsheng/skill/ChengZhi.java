            ...
            for (Player player : r.getGame().getPlayers()) {
                if (player instanceof HumanPlayer p) {
                    ...
                    if (p.equals(r)) {
                        ...
                        builder.setSeq(seq2);
                        GameExecutor.post(r.getGame(), () -> r.getGame().tryContinueResolveProtocol(r, Role.skill_cheng_zhi_tos.newBuilder().setEnable(false).setSeq(seq2).build()), builder.getWaitingSecond() + 2, TimeUnit.SECONDS);
                    }
