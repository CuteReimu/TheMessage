...
class GameExecutor private constructor() : Runnable {
    ...
    override fun run() {
        while (true) try {
            ...
            GameExecutor.Companion.log.error("take queue interrupted", e)
            Thread.currentThread().interrupt()
        } catch (e: Exception) {
            GameExecutor.Companion.log.error("catch throwable", e)
        }
    }
    ...
    private fun post(gameAndCallback: GameAndCallback) {
        try {
            ...
            GameExecutor.Companion.log.error("put queue interrupted", e)
            Thread.currentThread().interrupt()
        }
    }

    private class GameAndCallback(game: Game, callback: Runnable) {
        val game: Game
    ...
    companion object {
        ...
        fun post(game: Game, callback: Runnable) {
            val mod: Int = game.id % GameExecutor.Companion.executors.size
            if (GameExecutor.Companion.executors.get(mod) == null) {
                synchronized(GameExecutor::class.java) {
                    if (GameExecutor.Companion.executors.get(mod) == null) {
                        GameExecutor.Companion.executors.get(mod) = GameExecutor()
                        val thread: Thread = Thread(GameExecutor.Companion.executors.get(mod))
                        thread.isDaemon = true
                        thread.start()
                    }
                }
            }
            GameExecutor.Companion.executors.get(mod).post(GameAndCallback(game, callback))
        }
        ...
        fun post(game: Game?, callback: Runnable?, delay: Long, unit: TimeUnit?): Timeout {
            return GameExecutor.Companion.TimeWheel.newTimeout(TimerTask { timeout: Timeout? ->
