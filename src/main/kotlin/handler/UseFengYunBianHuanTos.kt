...
class UseFengYunBianHuanTos : AbstractProtoHandler<Fengsheng.use_feng_yun_bian_huan_tos>() {
    override fun handle0(r: HumanPlayer, pb: Fengsheng.use_feng_yun_bian_huan_tos) {
        ...
        if (card.type != Feng_Yun_Bian_Huan) {
            logger.error("这张牌不是风云变幻，而是$card")
            r.sendErrorMessage("这张牌不是风云变幻，而是$card")
            return
        }
        if (card.type != Feng_Yun_Bian_Huan) card = card.asCard(Feng_Yun_Bian_Huan)
