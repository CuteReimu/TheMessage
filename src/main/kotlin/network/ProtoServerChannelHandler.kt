...
class ProtoServerChannelHandler : SimpleChannelInboundHandler<ByteBuf>() {
    override fun channelActive(ctx: ChannelHandlerContext) {
        ...
        ProtoServerChannelHandler.Companion.log.info(
            "session connected: " + channel.id().asShortText() + " " + channel.remoteAddress()
        )
        val player = HumanPlayer(channel)
        if (ProtoServerChannelHandler.Companion.playerCache.putIfAbsent(channel.id().asLongText(), player) != null) {
            ProtoServerChannelHandler.Companion.log.error("already assigned channel id: " + channel.id().asLongText())
        }
    }
    ...
    override fun channelInactive(ctx: ChannelHandlerContext) {
        ...
        val player: HumanPlayer = ProtoServerChannelHandler.Companion.playerCache.remove(channel.id().asLongText())
        if (player == null) {
            ProtoServerChannelHandler.Companion.log.error("already unassigned channel id: " + channel.id().asLongText())
            return
        }
        ...
        synchronized(Game::class.java) {
            if (game.isStarted) {
                ...
                ProtoServerChannelHandler.Companion.log.info(player.playerName + "离开了房间")
    ...
    public override fun channelRead0(ctx: ChannelHandlerContext, msg: ByteBuf) {
        ...
        if (msgLen < 2) {
            ProtoServerChannelHandler.Companion.log.error("incorrect msgLen: " + msg.readableBytes())
            ctx.close()
            return
        }
        val id = msg.readShortLE()
        val protoInfo: ProtoServerChannelHandler.ProtoInfo = ProtoServerChannelHandler.Companion.ProtoInfoMap.get(id)
        if (protoInfo == null) {
            ProtoServerChannelHandler.Companion.log.error("incorrect msg id: $id")
            ctx.close()
            return
        }
        ...
        if (id != ProtoServerChannelHandler.Companion.heartMsgId && id != ProtoServerChannelHandler.Companion.autoPlayMsgId) {
            ProtoServerChannelHandler.Companion.log.debug(
                "recv@%s len: %d %s | %s".formatted(
                    ctx.channel().id().asShortText(), msgLen - 2, protoInfo.name,
                    ProtoServerChannelHandler.Companion.printer.printToString(message).replace("\n *".toRegex(), " ")
                )
            )
        }
        val player: HumanPlayer = ProtoServerChannelHandler.Companion.playerCache.get(ctx.channel().id().asLongText())
        if (!player.limiter.allow()) {
            ProtoServerChannelHandler.Companion.log.error("recv msg too fast: " + ctx.channel().id().asShortText())
            ctx.close()
            return
        }
        ...
        if (handler != null) {
            ...
            ProtoServerChannelHandler.Companion.log.warn("message: " + protoInfo.name + " doesn't have a handler")
        }
    }
    ...
    companion object {
        ...
        private val ProtoInfoMap: Map<Short, ProtoServerChannelHandler.ProtoInfo> = HashMap()
        private val playerCache: ConcurrentMap<String, HumanPlayer> = ConcurrentHashMap()
        private val heartMsgId: Short = ProtoServerChannelHandler.Companion.stringHash("heart_tos")
        private val autoPlayMsgId: Short = ProtoServerChannelHandler.Companion.stringHash("auto_play_tos")
        fun exchangePlayer(oldPlayer: HumanPlayer, newPlayer: HumanPlayer) {
            oldPlayer.channel = newPlayer.channel
            if (ProtoServerChannelHandler.Companion.playerCache.put(
                    newPlayer.channel.id().asLongText(),
                    oldPlayer
                ) == null
            ) {
                ProtoServerChannelHandler.Companion.log.error(
        ...
        init {
            try {
                ProtoServerChannelHandler.Companion.initProtocols(Fengsheng::class.java)
                ProtoServerChannelHandler.Companion.initProtocols(Role::class.java)
        ...
        private fun initProtocols(protoCls: Class<*>) {
            ...
            for (d in descriptor.messageTypes) {
                ...
                val id: Short = ProtoServerChannelHandler.Companion.stringHash(name)
                if (id.toInt() == 0) {
                ...
                if (ProtoServerChannelHandler.Companion.ProtoInfoMap.putIfAbsent(
                        id,
                        ProtoServerChannelHandler.ProtoInfo(name, parser, handler)
                    ) != null
                ) {
