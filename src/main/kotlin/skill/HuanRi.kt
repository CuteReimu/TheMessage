...
class HuanRi : AbstractSkill(), TriggeredSkill {
    ...
    override fun execute(g: Game): ResolveResult? {
        ...
        if (fsm.cardType != Common.card_type.Diao_Bao && fsm.cardType != Common.card_type.Po_Yi) return null
        if (!fsm.player.isRoleFaceUp()) return null
        fsm.askWhom.addSkillUseCount(skillId)
        HuanRi.Companion.log.info(fsm.askWhom.toString() + "发动了[换日]")
        for (p in g.players) {
            (p as? HumanPlayer)?.send(
                Role.skill_huan_ri_toc.newBuilder().setPlayerId(p.getAlternativeLocation(fsm.askWhom.location()))
                    .build()
            )
        }
