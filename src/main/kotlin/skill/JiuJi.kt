...
class JiuJi : AbstractSkill(), TriggeredSkill {
    ...
    override fun execute(g: Game): ResolveResult? {
        if (g.fsm !is OnUseCard || fsm.askWhom.findSkill<Skill>(skillId) == null || !fsm.askWhom.isAlive()) return null
        if (fsm.cardType != Common.card_type.Shi_Tan && fsm.cardType != Common.card_type.Wei_Bi && fsm.cardType != Common.card_type.Li_You) return null
        if (fsm.targetPlayer != fsm.askWhom) return null
        if (fsm.targetPlayer.isRoleFaceUp()) return null
        fsm.askWhom.addSkillUseCount(skillId)
        JiuJi.Companion.log.info(fsm.askWhom.toString() + "发动了[就计]")
        for (p in g.players) {
            (p as? HumanPlayer)?.send(
                Role.skill_jiu_ji_a_toc.newBuilder().setPlayerId(p.getAlternativeLocation(fsm.askWhom.location()))
                    .build()
            )
        }
